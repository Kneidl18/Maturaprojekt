...
#define SCL_PIN		PTDD_PTDD4	  // Clock= PC0		
#define SCL_PORT	PTDDD_PTDDD4



#define SDA_PIN		PTDD_PTDD3	  // Data= PC1		
#define SDA_PORT	PTDDD_PTDDD3

#define READ_ADRESS_AD7415 0x91  // war 0x91
#define WRITE_ADRESS_AD7415 0x90  // war 0x90
...
void Init_AD7415 (void)
{
	...
	AirDelay = 0;
 	...
 	IIC_Start();
    IIC_Send_Byte (WRITE_ADRESS_AD7415);       // Adresse (1001 001 0) (Write)
	Reply= IIC_Get_Ack();
...
void IIC_Start (void) {
	...
	//Nop();			     	// Zuerst DATA auf LOW, danach Clock (hier nicht. nur data low)
...
/****************************************************************************/
/*                                                                          */
/*  IIC_STOP: Stop-Kondition: Zuerst geht Clock auf LOW, danach DATA        */
/*  	      auf HIGH, dann clock auf HIGH				    */
/*                                                                          */
/****************************************************************************/
...
void IIC_Stop (void){
	...
	SCL_PORT = 1;			// danach CLOCK auf LOW
	SCL_PIN = 0;

	SDA_PORT = 0;			// Data auf HIGH
...
Byte Counter,ShiftWert;
	...
	for (Counter=0; Counter <8; Counter++){
		...
		SCL_PORT = 1;		// Einen Clock geben  --> Clock auf LOW
		SCL_PIN = 0;
		Nop();
		Nop();
		Nop();
...
unsigned char Response;
    ...
    SCL_PORT = 1;    		// Einen Clock geben  --> SCL = LOW
    SCL_PIN = 0;
	...
	SCL_PORT = 0;        // Clock wieder auf HIGH
...
void Read_Air_Temp(void)
{
  ...
  {                                 // Auslesen einmal pro Sekunde, weil öfter eh kaum Sinn macht...
	...
	IIC_Start ();				// Read!!
	IIC_Send_Byte (READ_ADRESS_AD7415);
	Reply= IIC_Get_Ack();
