...
#define MODULE_RELAISE		SENSOR_RELAISE
//************************************************************************************************
//*                                                                                              *
...
int Relaise_CheckState(void)
{
	...
	//invertet logic -> when relaiseSwitch closed then logic 0 on PORT
	//only 99% sure
...
{
	CC2500_sendPacketArr[0] = 4;
	CC2500_sendPacketArr[1] = PCB << 4;
	CC2500_sendPacketArr[1] += MODULE_RELAISE;
	CC2500_sendPacketArr[2] = (byte)(RelaiseState >> 8);
	CC2500_sendPacketArr[3] = (byte)(RelaiseState & 0x00FF);
}