...
bool BME680_Class::commonInitialization()
{
  ...
  */
  ...
  if (_I2CAddress == 0 &&
      bitRead(SPI_Register, BME680_SPI_MEM_PAGE_POSITION))
  {                                                          // Wrong mode for ID
...
void BME680_Class::getCalibration()
{
  ...
  *************************************/
  ...
  for(int i = 0; i < sizeof(coeff_arr1); i++){
  Serial.print(coeff_arr1[i]);
  Serial.print(", ");
  }
  for(int i = 0; i < sizeof(coeff_arr2); i++){
  Serial.print(coeff_arr2[i]);
  Serial.print(", ");
  }
  ...
  ************************************/
  ...
  Serial.print(_res_heat_range);
  Serial.print(", ");
  Serial.print(_res_heat);
  Serial.print(", ");
  Serial.print(_rng_sw_err);
  Serial.print(", ");
  Serial.println();
...
bool BME680_Class::setGas(uint16_t GasTemp, uint16_t GasMillis) const
{
  ...
  else
  {
    ...
    Serial.print("heatr_res: ");
    Serial.println(heatr_res);
